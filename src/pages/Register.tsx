...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Step 2: Make sure we got a real user back
      if (!data.user) {
      ...
      if (profileError) {
        // If profile insert fails, clean up is hard — just show the error
        console.error('Profile insert error:', profileError);
        throw new Error('Account created but profile setup failed: ' + profileError.message);
      }

      // Step 4: Navigate directly — no alert needed
      navigate('/login', {
    ...
    <>
      ...
      <div className="max-w-lg mx-auto px-6">
        <div className="bg-white rounded-3xl shadow-2xl p-10">
          ...
          {/* Role Tabs */}
          <div className="flex border-b mb-8 bg-zinc-50 rounded-xl p-1">
            {[
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-4 bg-blue-950 hover:bg-blue-900 text-white rounded-2xl font-semibold text-base transition flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed mt-2"
            >
